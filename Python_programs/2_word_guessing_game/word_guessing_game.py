...
def get_word():
    return random.choice(word_list)    # выбираем одно слово из списка, преобразуем в строку

def display_hangman(tries_local):
    stages = stages = [  # финальное состояние: голова, торс, обе руки, обе ноги
                '''
                   --------
                   |      |
                   |      O
    ...
    return stages[tries_local]                                      # Возвращаем рисунок висельника в зависимости от количества попыток

def play():
    print('Давайте играть в угадайку слов!')
    tries = 6                                                       # Начальное количество попыток: 6                            
    print(display_hangman(tries))                                   # Начальный рисунок виселицы
    word_completion = '_ '*len(word)                                # Строка, которую видит пользователь
    word_completion_list = word_completion.split()                  # Превращение в побуквенный список
    past_letters = []                                               # Список, куда будем складывать буквы, которые уже были
    print(word_completion)
    while tries >= 0:                                               # Основной цикл
        
        flag = False                                                # Защита от неверных форматов
        while flag == False:
            s = input('Введите одну строчную букву русского алфавита: ')
            if len(s) == 1 and ord(s) >= 1072 and ord(s) <= 1103:
                flag = True
            else:
                flag = False
        
        
        if s in word and s not in past_letters:                     # Проверка точно ли пользователь ввел одну русскую строчную букву
            for i in range(len(word)):
                if word[i] == s:                                    # Проверка, есть в загаданном слове данная буква. 
                    word_completion_list[i] = s                     # Замена черточек на буквы, если пользователь отгадал
            print('Есть такая буква. Откройте букву: ', s)
            print(word_completion_list)       
            past_letters.append(s)                                  # Добавление отработанный буквы в список, чтобы в дальнейшем подскаывать пользователю не повторяться
            if '_' not in word_completion_list:
                print('Поздравляю, вы угадали слово!')              # Если все черточки были заменены на буквы, пользователь победил
                break                                               # Цикл прерывается
            
        elif s in past_letters:
            print('Вы уже вводили данную букву. Не тратьте попытку зря, выберите другую букву')
            
        else:
            print('Нет такой буквы')
            past_letters.append(s)                                  # Добавление отработанный буквы в список, чтобы в дальнейшем подскаывать пользователю не повторяться
            tries -= 1                                              # Если пользователь ошибся, количество попыток уменьшается на одну
            print(display_hangman(tries))                           # Рисуется обновлённая висельница
            
            if tries == 0:
                print('К сожалению, вы проиграли. Попробуйте еще раз!') # Если попытка закончились, пользователь проиграл
                break                                                   # Цикл прерывается
